class Renderer {
    ...
    draw(shader, geometry) {
        ...
        this.savedProgram._id = _gl.getParameter(_gl.CURRENT_PROGRAM);
        _gl.viewport(0, 0, window.innerWidth * window.devicePixelRatio, window.innerHeight * window.devicePixelRatio);
        ...
        for (let key in shader.uniforms) {
            ...
            switch (uni.type) {
                ...
                case 't':
                    ...
                    _gl.activeTexture(_gl[`TEXTURE${texIndex}`]);
                    _gl.bindTexture(_gl.TEXTURE_2D, texture._gl);
                    _gl.uniform1i(uLoc, texIndex);
                    if (texture.needsUpdate || texture.dynamic) texture.texImage2D();
                    ++texIndex;
                break;
            }
        }

        let isBlending = _gl.isEnabled(_gl.BLEND);
        let currentDepthMask = _gl.getParameter(_gl.DEPTH_WRITEMASK);
        ...
        _gl.depthMask(currentDepthMask);
        if (isBlending) _gl.enable(_gl.BLEND);
        _gl.useProgram(this.savedProgram);
    }
}
